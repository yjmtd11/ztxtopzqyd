...
多个请求体时用'&'号或者换行隔开" ‼️
增加文件读取body 20210910
文件名：YOUTH_READ.txt,放在scripts/，换行符分隔
*/
...
if (!$.isNode() && !YouthBody == true) {
    ...
    if ($.isNode()) {
		...
		//增加文件读取body 20210910
		var fs = require("fs");
		zq_youthbody = fs.readFileSync("YOUTH_READ.txt", "utf8");
		if (zq_youthbody !== `undefined`) {
			YouthBodys = zq_youthbody.split("\n");
			console.log(`您选择的是文件方式\n`);
		} 
    } else if (!$.isNode() && YouthBody.indexOf("&") > -1) {
    ...
    return new Promise((resolve, reject) => {
        $.post(batHost('user/stay.json', timebodyVal), (error, resp, data) => {
            ...
            if (timeres.error_code == 0) {
				...
				//add at 20210913
				if (timeres.time >= Math.floor(Math.random() * (7200 - 9000) + 9000)) {
					// 时长达2小时+，设置下次执行不上传时长
						addtime=false;
					}
            }
